...
void i2c_read_bytes(uint8_t slave_addr, uint8_t register_addr, uint8_t length, uint8_t * data) {
  ...
  //            uint8_t length          -       amount of data to be written
  //
  //Outputs:    N/A
  //
  //Description: Given the slave_addr and register_addr, this function writes data to
...
PROCESS_THREAD(mpu6050_process, ev, data)
{
  ...
  while (1) {
    ...
    printf("\t\tSENSITIVITY INFORMATION\n\n");
    ...
    printf("Accelerometer Sensitivity: %d\n", ( (accel_fs & 0x18) >> 3));

    printf("Accelerometer Readings (X Y Z):");
    for(i=0;i<DATA_SIZE;i++){
      printf(" %d",accel[i]);
    }
    printf("\n");

    printf("Gyroscope Readings (X Y Z):");
    for(i=0;i<DATA_SIZE;i++){
      printf(" %d",gyro[i]);
    }
    printf("\n");
