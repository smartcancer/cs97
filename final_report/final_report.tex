...
gait/motion detection.  However, WBAN, unlike the other research we found, is capable of measuring your "fitness level" while exercising.
...
individual's runs. Another application is /textit{Zombies, Run!} which has 
...
social media. The /textit{Zombies, Run!} application has shown to be successful
by popular account and has therefore involved encouraged many people
to exercise more \cite{sixtostart:zrun}. Zombies, Unfortunately, the /textit{Zombies, Run!} application is extremely
...
famous social networking applications /cite{mulas:race}. In \textit{Everywhere Race!}, people were allowed to connect with other users in a virtual run.  This virtual run would allow multiple connected users to run a specified distance in different locations while still being able to compete in a virtual race with each other.  However, all of these fitness apps share one thing in common: they only measure running data; as explained, /textit{Zombies, Run!} and \textit{Everywhere Race!} is only capable of encouraging a user to run; furthermore, the researchers data did not involve applications that do more than just tracking running exercise data.  UF!t offers a variety exercises, so that
