...
replacing usual forms of exercising \cite{jeffords:obesity}. In earlier years, traversing long
...
Practically, the classifier considers a fixed number 
...
After clusters are determined, we classify incoming data
...
The motes communicate with a base station based on a passive
...
needed to listen for a response \cite{buettner:xmac}.  uF!t is a device that
will be used on the go with high respect for portability.  To
ensure its portability, it is important to maintain a high life
expantancy for the battery pack. One of the largest energy consumers in
a wireless sensor network is radio communication, so by limiting the amount time
needed to listen to responses, like in X-mac, it will ensure that
we are consuming as little energy as possible when engaging in
radio communication.
...
We note that we note that k-means is a naïve solution that
