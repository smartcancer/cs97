...
defined as completing 1 situp in 3 seconds or more. From our evaluations, 
...
small pockets of time to exercise. Therefore, uF!T intends to assist an individual with a busy lifestyle maintain a healthy lifestyle.  Researchers have found that devices that monitor a person's vitals or exercise information are conducive to leading a healthy lifestyle \cite{acharya:selfmonitor}.  Currently, people
are able to record their exercises by on paper or on a spreadsheet.  However, both of these methods are prone to human
...
The uF!T exercise counter records the number of repetitions completed for an exercise by counting defined features of the exercise.
...
found from our situp detector, we set threshold values that
would represent the minimum angle value to be considered a rest
...
corset to the uF!t's casing (see Figure 3).  The elastic qualities of the neoprene
...
provide more promising results (we would use a discretized model to update the current angle). Therefore, angle accuracy would be solely dependent on the fidelity of the gyroscope measurements. For the gyroscope, we used 
...
In our time constant, we used a time constant of \tau=\textit{.5
...
take precedence over the gyroscope.  Since the gyroscope drifts over time, under shorts periods of time (such as a quick rise during a situp) measurements are
...
represents the threshold value for rests. Statically defining threshold values is inflexible when detecting situps given that an individual is likely to vary their situp form.  However, we found threshold values provided
